...
**Camera scanning** uses the on-device ML Kit Document Scanner (Android) / VisionKit (iOS) via `cunning_document_scanner`. On Android this requires **Google Play services** to be present and up to date — the scanner module is delivered through it. On a device/emulator without current Play services the camera scanner can't open; the app now **surfaces a clear error** ("Couldn't open the scanner. Make sure Google Play services is installed and up to date") instead of appearing to do nothing, and **gallery import** remains available as a fallback path that doesn't need Play services.
